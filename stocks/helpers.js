...
Handlebars.registerHelper('jsStringEscape', function (string) {
...
Handlebars.registerHelper('getJSON', function (data) {
  const validDataStr = jsStringEscape(JSON.stringify(data))
  return `'${validDataStr}'`
})

Handlebars.registerHelper('getPeerNameSectionName', function (peerName, sectionName) {
  return `${peerName}-${sectionName}`
})

Handlebars.registerHelper('getLastScoreOfPeer', function (scoresByPeer, peerName) {
...
Handlebars.registerHelper('addClassIfEqual', function (value1, value2, className, alternativeClassName) {
  if (value1 === value2) {
    return className
  } else {
...
Handlebars.registerHelper('addAnalystsRecommendationClass', function (value) {
  const classMap = {
...
Handlebars.registerHelper('getPeerScoreAtTime', function (scoresByPeer, peer, exchangeDate, byPastMonths) {
...
Handlebars.registerHelper('getAverageResultsForAnalysis', function (analysisByPeer) {
...
Handlebars.registerHelper('getItemsGroupedBy', function (items, groupCount) {
  const results = []
  let newItem = []
